...
export function ProductSetOptions({
  ...
  const [rows, setRows] = useState<Array<{ label: string; price: string }>>(() =>
    saved.map((o) => ({ label: o.label, price: String(o.price / 100) })),
  )
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [notice, setNotice] = useState<string | null>(null)

  function patch(index: number, change: Partial<{ label: string; price: string }>) {
  ...
  async function save() {
    ...
    const options = rows
      .map((row) => ({ label: row.label.trim(), price: Math.round(Number(row.price) * 100) }))
  ...
  const dirty =
    ...
    rows.some(
      (row, i) =>
        row.label !== saved[i]?.label || Math.round(Number(row.price) * 100) !== saved[i]?.price,
    )
      ...
      <div className="mb-1 flex items-start justify-between gap-4">
        ...
        {rows.length < 8 && (
          <Button
            size="sm"
            variant="outline"
            onClick={() => setRows((current) => [...current, { label: '', price: '' }])}
          >
